...
import styles from "./Projectt.module.css";
...
function Project() {
  ...
  useEffect(() => {
    setTimeout(() => {
      fetch(`http://localhost:5000/projects/${id}`, {
        ...
        .then((resp) => resp.json())
        .then((data) => {
          const costOfServices = data.services.reduce(
            (acc, service) => acc + parseFloat(service.cost),
            0
          );

          setProject({ ...data, cost: costOfServices });
  ...
  function createService() {
    setMessage("");
    // last service
    const lastService = project.services[project.services.length - 1]; // serve para pegar o ultimo serviço adicionado

    lastService.id = uuidv4(); // converte o custo do serviço para number
